...
3. 以降 $D + D \bmod 2 = X$ とおく。以下が成立することを示す ※ $s, t$ は非負整数
...
- 分裂遷移の遷移先は $g(1) \oplus g(4k), g(2) \oplus g(4k-1), ... , g(4k) \oplus g(1)$ と表せるが、仮定よりこれらは全て以下のいずれかのパターンで表せる ($s, t$ は非負整数)
...
- したがって、 $\lbrace 0, 1, 2, 4, 3, ... , 4k \rbrace$ には到達できるが、 $4k + 1$ には到達できないため、 $g(4k + 1) = 4k + 1$ が成立する。

#### $g(4k+2)$

- 攻撃遷移は、仮定より $\lbrace g(v) | 0 \leq v \leq 4k + 1 \rbrace = \lbrace 0, 1, 2, 4, 3, ... , 4k, 4k + 1 \rbrace$ の状態に到達可能である。
- 分裂遷移の遷移先は $g(1) \oplus g(4k + 1), g(2) \oplus g(4k), ... , g(4k + 1) \oplus g(1)$ と表せるが、仮定よりこれらは全て以下のいずれかのパターンで表せる ($s, t$ は非負整数)
...
- 分裂遷移の遷移先は $g(1) \oplus g(4k + 2), g(2) \oplus g(4k + 1), ... , g(4k + 2) \oplus g(1)$ と表せるが、仮定よりこれらは全て以下のいずれかのパターンで表せる ($s, t$ は非負整数)
...
- 分裂遷移の遷移先は $g(1) \oplus g(4k + 3), g(2) \oplus g(4k + 2), ... , g(4k + 3) \oplus g(1)$ と表せるが、仮定よりこれらは全て以下のいずれかのパターンで表せる ($s, t$ は非負整数)
...
- $x \oplus y = 0$ となるには、 $x = y$ が必要十分条件だが、 $D+1 ≡ 1 \pmod 2$ なので、分裂の遷移で到達する状態の Grundy 数は $g(2s) \oplus g(2t + 1) \neq 0$ ($s, t$ は非負整数) となるため、 $0$ に到達不可能。
...
- スライムの体力が $D + 1$ の場合、攻撃遷移で到達可能な $\lbrace g(k) | 1 \leq k \leq D, k は整数 \rbrace = \lbrace 1, 2, \cdots , D - 1, D \rbrace$ で、 $0$ が存在しないため、分裂遷移で $0$ に到達可能かを考える。
  ...
  - 上記より、攻撃遷移と分裂遷移を考慮すると $g(D + 1) = D + 1$ が成り立つ。

- スライムの体力が $D + 2$ の場合、攻撃遷移で到達可能な $\lbrace g(k) | 1 \leq k \leq D + 1, k は整数 \rbrace$ に $0$ が存在しないため、分裂遷移で $0$ に到達可能かを考える。

  - $x \oplus y = 0$ となるには、 $x = y$ が必要十分条件だが、 $D+2 ≡ 1 \pmod 2$ なので、分裂の遷移で到達する状態の Grundy 数は $g(2s) \oplus g(2t + 1) \neq 0$ ($s, t$ は非負整数) となるため、 $0$ に到達不可能。
...
以降 $D + D \bmod 2 = X$ とおく。以下が成立することを示す ※ $s, t$ は非負整数
...
- またこれは、先頭 $X$ 項の周期と同じであり、同様に考えるとこの s に関わる周期も $D + D \bmod 2$ 項であることが言える。
